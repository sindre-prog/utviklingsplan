...
requireText(app, /function experimentReviewSpec[\s\S]*el\("details", \{ class: "experiment-review-details"[\s\S]*"Se tilbake og juster"/, "Eksperimentrefleksjonen vises ikke progressivt etter opprettelse.");
requireText(app, /function experimentEditorSpecs[\s\S]*Navn på eksperimentet[\s\S]*required: true[\s\S]*Hva vil du prøve\?[\s\S]*Hvor skal du prøve det\?[\s\S]*Når vil du se tilbake\?[\s\S]*Hva skal du se etter\?[\s\S]*experimentContextSpec/, "Felles eksperimentopprettelse mangler obligatorisk navn eller den avtalte, lette arbeidsflyten.");
requireText(app, /function focusViewDescription[\s\S]*Hva vil du bli bedre på\?[\s\S]*Den indre utviklingslinjen:[\s\S]*leder deg selv og andre[\s\S]*Hvor skal utviklingen merkes\?[\s\S]*Den ytre utviklingslinjen:[\s\S]*konkrete strategiske prioriteringer[\s\S]*Hva vil du prøve i praksis\?[\s\S]*Små atferdsforsøk du prøver, observerer og justerer\./, "Fokusarbeidsflatene mangler avtalte spørsmål og forklaringer.");
requireText(app, /function focusHubIntro[\s\S]*Utviklingsfokus[\s\S]*Hva skal du utvikle\?[\s\S]*Velg tre lederkompetanser som gir retning for utviklingen[\s\S]*Én kan være hovedfokus nå\./, "Fokus mangler avtalt samlende introduksjon.");
requireText(app, /function defaultWorkspacePane\(\) \{[\s\S]*return "now";/, "Klientplanens standardinngang er ikke Akkurat nå.");
requireText(app, /function clientWorkspaceTabs[\s\S]*workspace-tab-count[\s\S]*workspace-tab-new-dot/, "Ressursfanen mangler antall eller signal om ny ressurs.");
requireText(app, /function focusDetail[\s\S]*Hva skal du oppnå – eller hva skal bli annerledes\?[\s\S]*Hvor skal forskjellen merkes – og for hvem\?/, "Fokusoppdraget mangler de presiserte planfeltene.");
