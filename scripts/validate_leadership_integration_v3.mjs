...
requireText(app, /function focusViewDescription[\s\S]*Hva vil du bli bedre på\?[\s\S]*Den indre utviklingslinjen:[\s\S]*Hvor skal utviklingen merkes\?[\s\S]*Det ytre arbeidsoppdraget:[\s\S]*Hva vil du prøve i praksis\?[\s\S]*Små atferdsforsøk du prøver, observerer og justerer\./, "Fokusarbeidsflatene mangler avtalte spørsmål og forklaringer.");
requireText(app, /function focusHubIntro[\s\S]*Hva skal du utvikle\?[\s\S]*Velg én lederkompetanse som hovedfokus[\s\S]*Fokusoppdrag viser hvor utviklingen skal merkes\./, "Fokus mangler avtalt samlende introduksjon.");
requireText(app, /function defaultWorkspacePane[\s\S]*role === "client" \? "now" : "direction"/, "Klientens standardinngang er ikke Akkurat nå.");
